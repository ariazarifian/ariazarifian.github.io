...
function renderThailandEvidence(root){
  ...
  const wrap=document.createElement('div');wrap.dataset.countryEvidence='THA';
  const section=document.createElement('section');section.className='detail-section';
  section.innerHTML=`<h3>VIVRE & S’INSTALLER · REPÈRES SOURCÉS</h3><div class="metrics-pair"><div><small>RÉSIDENCE FISCALE</small><strong class="long">${esc(f.tax_residency.headline)}</strong><p class="scope">présence agrégée sur l’année</p></div><div><small>COURT SÉJOUR · PASSEPORT FR</small><strong class="long">${esc(f.residence_visa.headline)}</strong><p class="scope">tourisme · droit au travail distinct</p></div></div><p>${esc(f.tax_residency.summary)}</p><p>${esc(f.residence_visa.summary)}</p><div class="metrics-pair"><div><small>GRAND BANGKOK · MÉNAGE 2024</small><strong>30 464 THB</strong><p class="scope">dépense moyenne / mois</p></div><div><small>THAÏLANDE · MÉNAGE 2024</small><strong>22 282 THB</strong><p class="scope">dépense moyenne / mois</p></div></div><p>${esc(f.cost_context.summary)}</p><p><strong>Santé · ${esc(f.healthcare.headline)}.</strong> ${esc(f.healthcare.summary)}</p><p><strong>Sécurité · ${esc(f.safety_context.headline)}.</strong> ${esc(f.safety_context.summary)}</p>`;
  ...
  wrap.append(section);anchor.before(wrap,details);
}
