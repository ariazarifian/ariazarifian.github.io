...
(()=>{
  ...
  function reconcileCanadaComparator(){
    const core=window.ATLAS_COUNTRY_EVIDENCE;
  ...
  function loadSingaporeEvidence(){
    if(window.ATLAS_COUNTRY_EVIDENCE_SINGAPORE){bindSingaporeComparator();reconcileSingaporeComparator();loadCanadaEvidence();return;}
    const existing=document.querySelector('[data-atlas-country-evidence-singapore]');
    if(existing){existing.addEventListener('load',()=>{bindSingaporeComparator();reconcileSingaporeComparator();loadCanadaEvidence();},{once:true});return;}
    ...
    singapore.addEventListener('load',()=>{bindSingaporeComparator();reconcileSingaporeComparator();loadCanadaEvidence();},{once:true});
  ...
  // PEX-D1B/D1C/D1D/D1E/D1F/D1G: keep the D1A evidence contract isolated, then load visible
