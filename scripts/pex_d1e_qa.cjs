...
console.log('PEX-D1E contract PASS');

const {chromium}=require('playwright');
const url='http://127.0.0.1:8000/atlas-exit/index.html';
const report={productHead:'37c60d7494463c2d137d5d4a0d4a984dcddc4beb',baseHead:'edcb964f27d7c05588fff6581684306946a25675',viewports:[],errors:[]};
...
(async()=>{
  ...
  for(const cfg of [{name:'desktop',width:1440,height:1000},{name:'mobile',width:390,height:844}]){
    ...
    assert.equal(panelStates.filter(x=>x==='READY_WITH_CAVEAT').length,6);
    assert.equal(panelStates.filter(x=>x==='READY_FOR_PRODUCT').length,2);
    ...
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),true);
    await page.screenshot({path:`qa-artifacts/pex-d1e-${cfg.name}.png`,fullPage:false});
    const full=await page.screenshot({path:`qa-artifacts/pex-d1e-${cfg.name}-full.png`,fullPage:true});
  ...
  fs.writeFileSync('qa-artifacts/pex-d1e-report.json',JSON.stringify(report,null,2));
